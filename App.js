...
const firebaseConfig = {...Constant.FIREBASE, ...Platform.OS === 'android' ? {appId: Constant.FIREBASE_SPECIFIC.appIdAndroid} : {appId: Constant.FIREBASE_SPECIFIC.appIdiOS}};
const App = () => {
	...
	return useObserver(() => (
		<PaperProvider theme={homeStore.darkMode ? CustomDarkTheme : CustomLightTheme}>
			<AppContainer
			...
			/>
			<ModalConnection isVisible={isVisible} onPress={() => { setIsVisible(false); }} />
			<ModalImage isVisible={homeStore.shouldShowImageZoom} uri={homeStore.imageHasToShown} />
		</PaperProvider>
	));
};
export default App;